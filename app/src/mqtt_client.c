...
static int MQTTKeepalive(iotx_mc_client_t *pClient)
{
    ...
    // log_info("Send Heartbeat Message == %s",pClient->buf_send);
...
static int iotx_mc_read_packet(iotx_mc_client_t *c, iotx_time_t *timer, unsigned int *packet_type)
{
    ...
    if ((rem_len > 0) && ((rem_len + len) > c->buf_size_read)) {
        log_info("mqtt read buffer is too short, mqttReadBufLen : %u, remainDataLen : %d", c->buf_size_read, rem_len);
...
static int iotx_mc_handle_recv_PUBLISH(iotx_mc_client_t *c)
{
    ...
    if (1 != MQTTDeserialize_publish((unsigned char *)&topic_msg.dup,
                                     &topic_msg.qos,//(int *)&topic_msg.qos,
    ...
    // log_info("topic_msg.payload == %s And The Len == %d",topic_msg.payload,topic_msg.payload_len);
#ifdef MQTT_ID2_CRYPTO
    ...
    // log_info("topic_msg.payload= | %s |,topic_msg.payload_len = | %d |", topic_msg.payload,topic_msg.payload_len);

    // log_info("msg.id = | %d |", topic_msg.packet_id);
    // log_info("topicName ssssssssssssssssssssssssssssssssssss= | %s |", tmp_topic_name);
	// log_debug("msg.id = | %s |", topic_msg.payload);
    // log_debug("msg.id = | %d |", topic_msg.packet_id);
    // log_debug("topicName ssssssssssssssssssssssssssssssssssss= | %s |", tmp_topic_name);
...
static int iotx_mc_cycle(iotx_mc_client_t *c, iotx_time_t *timer)
{
	...
	//log_debug("client state = %d", state);
    ...
    // read the socket, see what work is due
    memset(c->buf_read,0x0,c->buf_size_read);//添加处理 ，清除以前的接收信息
    rc = iotx_mc_read_packet(c, timer, &packetType);
    // log_info("Receive Date == %s, And Len == %d",c->buf_read,c->buf_size_read);
	if(packetType != 0 )
		log_debug("packetyype = %d\n",packetType);
    if (rc != SUCCESS_RETURN) {
    ...
    // log_info("Receive PackeType == %d",packetType);
...
static void iotx_mc_keepalive(iotx_mc_client_t *pClient)
{
	...
	if(currentState != 2)
	{
        log_info("currentState =================%d\n",    currentState);
        if(g_device_working == 1)

        {  
            currentState = 2;
            iotx_mc_set_client_state(pClient, IOTX_MC_STATE_CONNECTED); //解决断网ALI KEEPALIVE导致系统崩溃
                      //sys_taskSend(TASK_ONE,T1_SOFT_RESET,0,0,0);
            err_flag = 1;
        }
        // else
        // {
        //     log_info("MQTT Connection Error, Motor Is Not Working Device Will Restart");
        //     //sys_taskSend(TASK_ONE,T1_SOFT_RESET,0,0,0);
        // }
    }
    do {

        // log_info("000000000000000000000000000000000000");
        ...
        if (IOTX_MC_STATE_DISCONNECTED_RECONNECTING == currentState) {
            ...
            // log_info("2222222222222222222222222222");
            HAL_MutexLock(pClient->lock_generic);
            // log_info("3333333333333333333333333333333");
            pClient->ping_mark = 0;
            HAL_MutexUnlock(pClient->lock_generic);
            // log_info("end the HAL_MutexLock");
            rc = iotx_mc_handle_reconnect(pClient);
            if (SUCCESS_RETURN != rc) {
                log_info("reconnect network fail, rc = %d", rc);
            } 
			else {
...
static int MQTTPubInfoProc(iotx_mc_client_t *pClient)
{
    ...
    do {
        ...
        for (;;) {
            ...
            // log_info("ReSend Date = %s And Date Length %d",(char *)repubInfo->buf,repubInfo->len);
...
{
    int rc = FAIL_RETURN;
    // log_info("Begin iotx_mc_connect after data_recv");
    // /*if (NULL == pClient) {
    //     log_info("pClient is Null ");
    //     return NULL_VALUE_ERROR;
    // }*/
    // log_info("=====================");
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
	...
	log_debug("start iotx_mc_connect.");
...
int IOT_MQTT_KEEPALIVE_AND_CHECK(void *handle)
{
    int                 rc = SUCCESS_RETURN;
...
int IOT_MQTT_Yield(void *handle, int timeout_ms)
{
    ...
    // log_info("1111111111111111111111111111111111111111111111111111111111111111111111");
    iotx_time_init(&time);
    utils_time_countdown_ms(&time, timeout_ms);
    // log_info("2222222222222222222222222222222222222222222222222222222222222222222222");
    do {
...
int IOT_MQTT_Publish(void *handle, const char *topic_name, iotx_mqtt_topic_info_pt topic_msg)
{
 ...
 iotx_mc_state_t hdw_get_mqtt_client_state(iotx_mc_client_t  *pClient)
 {	
	return ( iotx_mc_get_client_state(pClient));
