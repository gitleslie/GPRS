...
char *LITE_json_value_of(char *key, char *src)
{
    ...
    src_iter = src;
    key_iter = key;
	log_debug("jsonnnnnnnn");
    do {
        if ((delim = strchr(key_iter, '.')) != NULL) 
		{
            key_len = delim - key_iter;

            log_debug("will malloc %d", key_len + 1);
    ...
    log_debug("before json_get_value_by_name");
    value = json_get_value_by_name(src_iter, strlen(src_iter), key_iter, &value_len, 0);
    if (NULL == value) {
        return NULL;
    }
    log_debug("after json_get_value_by_name, will malloc %d", (value_len + 1) * sizeof(char));
    ret = LITE_malloc((value_len + 1) * sizeof(char));
    if (NULL == ret) {
        return NULL;
    }
    log_debug("after LITE_malloc");
    LITE_snprintf(ret, value_len + 1, "%s", value);
    log_debug("after LITE_snprintf");
    log_debug("json = %s", ret);
