...
INT32 cmd_testpin(const INT8 *str);
INT32 cmd_openlog(const INT8 *str);
INT32 cmd_closelog(const INT8 *str);
void cmd_hdw_test(void);
INT32 cmd_testtimer(const INT8 *str);
INT32 cmd_mqttkey(const INT8 *str);
INT32 cmd_restart(const INT8 *str);
INT32 cmd_wsmac(const INT8 *str);
INT32 cmdcmp(INT32 i,const UINT8 *b,UINT16 b_len);
INT32 cmd_ping(const INT8 *str);
INT32 cmd_mipcall(const INT8 *str);
...
// static int cgsn_proc1(command_t *cmd, const char* resp)
// {
// //     if (strequsub("+MPING", resp)) {
...
//     return 0;
// }
static int cping_proc(command_t *cmd, const char* resp)
{
    if (strequsub("+MPING:", resp)) {
        // process here
        return 1;
...
static int cmip_proc(command_t *cmd, const char* resp)
{
    if (strequsub("+MIPCALL:", resp)) {
        // process here
        return 1;
    }
    else if (strequ("OK", resp)) {
...
INT32 cmd_ping(const INT8   *str)
{
    void *rv;
    INT8 ret,op;
    INT8 sip[20] = "139.196.135.135";
    char    cmd[80],cmd1[1024];
    memset(cmd1, 0, 1024);
    // ret = sget_response(str,"d100s",&op,sip);
    // log_info("%d %s",op,sip);
    op = 1;
    sprintf(cmd, "AT+MPING=%d,\"%s\"\r",op, sip);
    rv = send_command_wait(cmd,cmd1,cping_proc,10000);
    if(rv == NULL)
    {
        log_info("AT+MPING Return Error");
    }
    else
    {
         sys_debug(DB2, "+ok=%s\r\n", cmd1);
...
INT32 cmd_mipcall(const INT8 *str)
{
    ...
    op = 1;
    INT8 sip[20] = "cmnet";
    // send_command_wait("AT+MIPCALL=1,\"cmnet\"\r",NULL,NULL,TIMEOUT_UNIT);
    // ret = sget_response(str,"d100s",&op,sip);
    memset(cmd1, 0, 1024);
    sprintf(cmd, "AT+MIPCALL=%d,\"%s\"\r",op, sip);
    rv = send_command_wait(cmd,cmd1,cmip_proc,10000);
    if(rv == NULL)
    {
        log_info("AT+MIPCALL Return Error");
    }
    else
    {
        sys_debug(DB2, "+ok=%s\r\n", cmd1);
...
//     INT32 ret;
//     void *rc;
//     rv = send_command_wait("AT+MPING=1,"10.170.4.112"\r", str, cgsn_proc1, TIMEOUT_UNIT);
//     DB(DB2,"%s",rv);
// }
...
{
    INT32 ret;
    INT32 pin1, pin2, level;
    ret = sget_response(str,"ddd",&pin1, &pin2, &level);

    if(3 != ret) {
        sys_debug(DB2, "+ok=param error %d\r\n", ret);
    }
    else
    {
        if(pin1 == 22 && pin2 == 21)
        {
            ret = sys_gpio_set(GAPP_IO_0, level); //设置PIN22
            if(ret < 0) //设置失败
            {
                sys_debug(DB2, "+ok=fail 1\r\n");
            } else
            {

                ret = sys_gpio_get(GAPP_IO_6, &level); //读取PIN21
                if(ret < 0 )
                {
                    sys_debug(DB2, "+ok=fail 3\r\n");
                } else
                {
                    sys_debug(DB2, "+ok=%d\r\n", level);
                }
...
void test_timer_hanlder(void *arg)
{

    log_info(" ~~ stop sys_getSysTick = %d", sys_getSysTick());
}
...
INT32 cmd_testtimer(const INT8 *str)
{
    INT32 ret;
    INT32 timer;
    ret = sget_response(str,"d",&timer);

    log_info(" ~~ start sys_getSysTick = %d", sys_getSysTick());
    ...
    sys_timer_new(timer, test_timer_hanlder,NULL);
...
{
    INT32 ret;
    INT8 productkey[64]={0};
    INT8 devname[64]={0};
    INT8 devsecret[128] = {0};

    ret = sget_response(str,"64s64s128s",productkey,devname,devsecret);//return the parameter_count
    if(ret !=3 ){
        log_err("Pub Params error!!check it!");
        return ret;
    }
...
INT32 cmd_wsmac(const INT8 *str)
{
    ...
    ret = sget_response(str,"64s64s",pwd,mac);//return the parameter_count
    if(ret == 2 ){
        UINT32 opt;
        INT32 fd;
        INT32 len;
        ...
        fd = sys_file_open("mac.txt", opt);
        //DB(DB1,"WriteDevParamToFlash open %d",fd);

        len = sys_file_write(fd, mac, sizeof(mac));
        if(len != sizeof(mac))
        {
    ...
    } else
    {
        ...
        INT32 file_len;
        opt = FS_O_RDWR | FS_O_CREAT;
        fd1 = sys_file_open("mac.txt", opt); //
        //DB(DB1,"devpara open %d",fd1);
        if(0 <= fd1)
        {
            file_len = sys_file_read(fd1,mac,sizeof(mac));//文件名字长度不能大于12
            //DB(DB1,"devpara read %d",file_len);
            sys_file_close(fd1);    //每次读取完之后最好关闭文件
            if(file_len == sizeof(mac))
            {
                sys_debug(DB2, "+ok=%s\r\n", mac);
                //DB(DB1,"devpara read %d",file_len);
            } else {
